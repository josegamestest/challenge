...
local itens2={{"flag_point_3d","flag_point_3d.png","flag_point_3d.obj"},}
for i = 1, #itens2, 1 do
for e = 1, #colors, 1 do	
	
	minetest.register_craftitem("challenge:tool_"..itens2[i][1].."_"..colors[e][1], {
		stack_max = 1,
		description = "challenge:tool"..itens2[i][1].."_"..colors[e][1],
		...
		on_use =
			function(_, player, pointed_thing, pos)
				...
				if  minetest.get_node(position).name == "air" then
					local node_add=minetest.set_node(position, {name="challenge:"..itens2[i][1].."_"..colors[e][1]})
						local meta = minetest.get_meta(position)
						meta:set_string("infotext", (jogador))
						meta:set_string("owner", jogador)
						local timer = minetest.get_node_timer(position)
						timer:start(0.2)
					minetest.after(0, function() player:get_inventory():remove_item("main", "challenge:tool_"..itens2[i][1].."_"..colors[e][1]) end)
				end
			end,
	})

	minetest.register_node("challenge:"..itens2[i][1].."_"..colors[e][1],{
		description =itens2[i][1].."_"..colors[e][1],
		ight_source = 5,
		walkable=false,
		--on_rotate = true,
		...
		paramtype2 = "facedir",
		tiles = {itens2[i][2].."^[multiply:"..colors[e][2]},
		drawtype = "mesh",
		mesh = itens2[i][3],
		pathfinding= true,
		...
		paramtype = "light",
		drop="challenge:tool"..itens2[i][1].."_"..colors[e][1],
